...
const EXAMPLE_TXS = [
  { label: 'Safe SOL Transfer',  sig: '5j7s8K2mWxG4YkRpNqVeLHbzXdM3NfCsQwPvUhJaEtDrYi9oA1cBmFg6nReZkTuCvSxQoH8WpXlYzMnGbKd2e3f' },
  { label: 'NFT Mint',           sig: '3vZ9wLhNrYqXsM5PtGbKdJ2eUfC8oA4cBmFg1nReZkTuCvSxQoH7WpXlYzMn6bKd2e3f5j7s8K2mWxG4YkRpNqVe' },
  { label: 'Token Swap',         sig: '2eUfC8oA4cBmFg1nReZkTuCvSxQoH7WpXlYzMn6bKd5j7s8K2mWxG4YkRpNqVeLHbzXdM3NfCsQwPvUhJaEtDrYi' },
];
...
export default function Home() {
  ...
  return (
    <div className="sentinel-bg grid-pattern min-h-screen">
      ...
      {/* Main content */}
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px 80px', display: 'grid', gridTemplateColumns: result ? '1fr 1fr' : '1fr', gap: 24, transition: 'grid-template-columns 0.4s' }}>
        ...
        {activeTab === 'scan' && (
          <div className="card-glow" style={{ padding: '28px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
              ...
              <span style={{ marginLeft: 'auto', fontSize: '0.7rem', color: '#8892a4', fontFamily: 'JetBrains Mono' }}>Powered by Gemini AI</span>
            </div>

            {/* Wallet scan shortcut */}
            {walletAddress && (
              <div style={{ background: 'rgba(153,69,255,0.08)', border: '1px solid rgba(153,69,255,0.2)', borderRadius: 10, padding: '12px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: '0.8rem', color: '#c4b5fd' }}>Wallet connected — your recent txs are auto-fetched for scanning</span>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: '0.7rem', color: '#8892a4', marginBottom: 8, fontFamily: 'JetBrains Mono' }}>QUICK EXAMPLES:</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {EXAMPLE_TXS.map(ex => (
                  <button key={ex.label} onClick={() => { setSignature(ex.sig); analyze(ex.sig); }} style={{ padding: '5px 12px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.04)', color: '#a0aec0', fontSize: '0.72rem', cursor: 'pointer', transition: 'all 0.15s' }}
            ...
            <div style={{ marginTop: 24, paddingTop: 24, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              {[
                { n: '01', t: 'Fetch', d: 'Pull raw tx data from Helius RPC' },
                { n: '02', t: 'Parse', d: 'Extract programs, accounts, amounts' },
                { n: '03', t: 'AI Analyze', d: 'Gemini 1.5 Flash runs threat model' },
                { n: '04', t: 'Verdict', d: 'Risk score + plain English report' },
              ].map(step => (
          ...
          <div className="card-glow" style={{ padding: '28px', borderColor: risk.border }}>
            ...
            <div style={{ background: risk.bg, border: `1px solid ${risk.border}`, borderRadius: 14, padding: '20px 24px', marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ fontSize: '0.75rem', color: '#8892a4', marginTop: 4 }}>{result.analysisTime}ms scan • {result.aiModel}</div>
              </div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {result.redFlags.map((flag, i) => (
                    <div key={i} className="threat-tag" style={{ background: 'rgba(255,51,102,0.08)', color: '#ff8fa3', border: '1px solid rgba(255,51,102,0.2)' }}>
            ...
            {/* Programs */}
            {result.programsInvolved.length > 0 && (
              <div style={{ marginBottom: 20 }}>
            ...
            {/* Recommendation */}
            <div style={{ background: result.riskLevel === 'LOW' ? 'rgba(0,255,136,0.06)' : 'rgba(255,51,102,0.06)', border: `1px solid ${risk.border}`, borderRadius: 10, padding: '14px 16px' }}>
            ...
            {/* Solscan link */}
            <a href={`https://solscan.io/tx/${signature}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 16, color: '#8892a4', fontSize: '0.75rem', textDecoration: 'none', transition: 'color 0.2s' }}
          ...
          <div className="card-glow" style={{ padding: '28px', gridColumn: '1 / -1' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
              {[
                ...
                { title: '⚡ Helius RPC', body: 'Enterprise-grade Solana node from Helius delivers transaction data in under 200ms with 99.9% uptime.' },
                { title: '👻 Phantom Wallet', body: 'Connect your Phantom wallet to auto-fetch and scan your recent transactions for threats.' },
                ...
                { title: '🏆 Built for Colosseum', body: 'Submitted to the Colosseum Frontier Hackathon. AI × Solana security tooling for the next billion Web3 users.' },
              ].map(card => (
