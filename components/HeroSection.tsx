...
const DEMO_TX = '5J5uWNRaF5XMkS5o7Tp2vRDpGe3RdE8WvKrVBqF3P2T9AxC6HcXtPMqNdL4Z';
...
export default function HeroSection() {
  ...
  const tryDemo = () => {
    scrollToScanner();
    setTimeout(() => {
      // Works with ScannerPanel's textarea or input
      const input = document.querySelector('#scanner textarea, #scanner input') as HTMLInputElement | HTMLTextAreaElement;
      if (input) {
        const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value') ||
          Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, 'value');
        nativeInputValueSetter?.set?.call(input, DEMO_TX);
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '0 16px', textAlign: 'center', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 20 }}>
          <button
            ...
            style={{ padding: '14px 32px', borderRadius: 12, fontWeight: 800, fontSize: 15, background: 'linear-gradient(135deg,#9945FF,#14F195)', color: '#0a0a0f', border: 'none', cursor: 'pointer' }}
        ...
        <p style={{ marginTop: 16, fontSize: 12, color: 'rgba(255,255,255,0.3)' }}>
          New to crypto? <button onClick={() => document.getElementById('learn')?.scrollIntoView({ behavior: 'smooth' })} style={{ background: 'none', border: 'none', color: '#9945FF', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>Learn what this does →</button>
        </p>
      </div>
    </section>
  );
